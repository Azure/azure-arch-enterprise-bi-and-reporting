...
Job Manager's responsibilities fall into three broad areas: data ingestion coordination, Logical and Physical Data Warehouse state management (i.e. `Standby`, `Active` and `Load`), and coordination of Analysis Services tabular model building. This document discusses these areas in the context of Job Manager APIs.
...
(Epemenral Storage Accounts)[./1-Understanding%20ephemeral%20blobs.md] are managed by the Job Manager. At any point in time, clients can fetch the active Epemeral Blob storage account and its SAS token by calling `/StorageAccounts` OData API.
